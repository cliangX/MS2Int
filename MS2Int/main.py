...
def train(rank, world_size, Mamba_Config):
    ...
    for epoch in range(max_epochs):
        ...
        epoch_train_loss = epoch_loss_tensor.item() / (world_size * len(train_loader))
        if rank == 0:
            logging.info(f"average train loss: {epoch_train_loss}")
...
def validate(rank, world_size, model, val_loader, device):
    ...
    if rank == 0:
        val_progress = tqdm(val_loader, ncols=100, desc="Validation(VAT)      ")
    else:
