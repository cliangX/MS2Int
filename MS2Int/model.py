...
class BiDirectionMixerModel(nn.Module):
    def __init__(
        ...
        self.transformation_matrix = nn.Parameter(torch.randn(39, 40))
    ...
    def forward(
        ...
        **mixer_kwargs,
        ...
        for f_layer, b_layer, h_fc in zip(
            ...
            hidden_states = h_fc(
                torch.cat([hidden_states_f, hidden_states_b.flip([1])], dim=-1)
            )
...
class MambaLMHeadModel(nn.Module, GenerationMixin):
    def __init__(
        self, config: MambaConfig, initializer_cfg=None, device=None, dtype=None
    ) -> None:
        self.config = config
        ...
        self.lm_head = nn.Linear(d_model, 41, bias=False, **factory_kwargs)
