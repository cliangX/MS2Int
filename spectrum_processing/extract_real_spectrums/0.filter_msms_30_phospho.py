...
"""
过滤 msms.txt：
1) 不再按 Modifications 内容做筛选——保留所有修饰类型以及未修饰肽段
2) 仅保留 Length <= 30 的行（Length 自动转为数值，无法转换的行将丢弃）
3) 删除 Sequence 列包含 'U' 的行（'U' 代表硒，不参与标准蛋白质搜索）

用法示例：
python extract_real_spectrums/0.filter_msms_30_phospho.py \
  -i /path/to/msms.txt \
  -o /path/to/output/msms.filtered_phospho_sty_lenle30.txt
...
def build_output_path(input_path: str, output_path: str | None) -> str:
    ...
    if not ext:
        ext = ".txt"
    # 默认输出名改为 phospho_sty 以反映筛选条件
    return f"{root}.filtered_phospho_sty_lenle30{ext}"
...
def filter_chunk(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    # 现在不再依赖 Modifications 做筛选，仅要求 Length 和 Sequence 存在
    required = ["Length", "Sequence"]
    for col in required:
        if col not in df.columns:
            raise ValueError(f"输入文件缺少必要列: {col}")

    # 第一步：不再按 Modifications 做任何筛选，保留所有修饰/未修饰情况
    is_mod_ok = pd.Series(True, index=df.index)
    stats["removed_by_modifications"] = 0
    ...
    # 三个条件综合（此处 is_mod_ok 恒为 True，仅限制 Length 与 Sequence）
    mask = is_mod_ok & is_len_ok & is_seq_ok
...
def main():
    parser = argparse.ArgumentParser(
        description="过滤 msms：仅保留 Modifications 含 Phospho (STY) 且 Length<=30 的行"
    )
    parser.add_argument(
